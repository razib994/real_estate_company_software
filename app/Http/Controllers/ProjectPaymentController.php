...
//        }
    ...
    public function getdetailsParticular($id, $particular_id){
        $start_date  = \request()->get('start_date', date('Y-m-d'));
        $end_date  = \request()->get('end_date', date('Y-m-d'));
        $project = Project::find($id);
        $show = Item::where('id', $particular_id)->get();
        $today =  date("Y-m-d");
        if($today != $start_date ) {
            $project_payment = ProjectPayment::select('*')->where([
                ['project_id', '=', $project->id],
                ['item_name', '=', $particular_id]
        ...
        return view('backend.pages.particular.index',   [
            'project'           => $project,
            'project_payments'   =>$project_payment,
            'shows'             =>$show,
            'start_date'=>$start_date,'end_date'=>$end_date
        ]);
    ...
    public function createindivisualPDF($id, $particular_id, $start_date, $end_date) {
        $project = Project::find($id);
        $show = Item::where('id', $particular_id)->get();
        $today =  date("Y-m-d");
        if($today != $start_date ) {
        ...
        $pdf = PDF::loadView('backend.pages.particular.pdf', [
            'project'           => $project,
            'project_payments'   =>$project_payment,
            'shows'             =>$show,
            'start_date'=>$start_date,'end_date'=>$end_date])->setPaper('a4', 'landscape');
        // download PDF file with download method
        return $pdf->stream('Item Particular.pdf');
    }
