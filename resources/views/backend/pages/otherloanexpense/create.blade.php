...
@section('content')
    <div class="container-fluid">
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">  Create  Loan Refund </h1>
            @if(Auth::guard('admin')->user()->can('bank.view'))
            <a href="{{ route('othersloans.loanexpense', $othersloans->id) }}" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i
                    class="fas fa-eye fa-sm text-white-50"></i>  Loan Refund  List </a>
                @endif
        </div>
        <!-- DataTales Example -->
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary"> Loan Refund Create </h6>
            </div>
            <div class="card-body">
                @if ($errors->any())
                ...
                @endif
                <form action="{{route('othersloans.stored',$othersloans->id)}}" method="post">
                    @csrf
                    <input type="hidden" name="investor_id" value="{{$othersloans->id}}">
